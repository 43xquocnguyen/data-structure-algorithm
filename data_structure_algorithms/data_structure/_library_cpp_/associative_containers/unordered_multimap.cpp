...
Unordered Multimap:
    ...
    * Thuộc tính:
        ...
        + Unordered multimap là một hash table
        + Liên kết giữa key và value (Associative)
        + Using hash table (Unordered)
        + Mỗi key ánh xạ 1 value (Map)
        + Các key không cần phải duy nhất (Multiple equivalent keys)
    ...
    * Function member:
        ...
        - Constructor:
            + unordered_multimap(): khởi tạo với 0 element
            + unordered_multimap({key, value},...)
            + unordered_multimap(first, last): khởi tạo với range[first, last]
            + unordered_multimap(unordered_multimap): khởi tạo mà copy toàn bộ dữ liệu 
        ...
        - Capacity:
            ...
            + empty(): => unordered_multimap có rỗng? (true or false)
        ...
        - Elements lookup: 
            ...
            + equal_range(key): => pair<unordered_multimap::iterator, unordered_multimap::iterator> (first: same lower_bound | second: upper_bound)

        - Modifiers:
            + insert(it, {key, value})/(it, n, {key, value})/(it, <>)/(it, it1, it2): chèn phần tử
            + erase(it)/(first, last): xóa phần tử
            + swap(unordered_multimap): hoán đổi dữ liệu 2 unordered_multimap
        ...
        - Non-member function overloads:
            + operators(unordered_multimap): Toán tử quan hệ
            + swap(unordered_multimap): Đổi dữ liệu 2 unordered_multimap
